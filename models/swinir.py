...
class SwinIR(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        self.conv_first = nn.Conv2d(6, embed_dim, kernel_size=3)
    ...
    def forward(self, x):
        ...
        x_first = self.conv_first(self.pad(x))  # 64 1/2

        x_t = self.conv_after_body(self.forward_features(x_first)) + x_first  # 64 1/2
        x_c_1 = self.conv1(x)
        x_c_2 = self.conv2(x_c_1) + x_c_1
        x_c_3 = self.conv3(x_c_2) + x_c_2
        x_tc = self.a * x_t + self.b * x_c_3
        # x_t = self.fuse(torch.cat((x_t, x2), dim=1))
        out_lf = self.after(self.up(x_tc))
